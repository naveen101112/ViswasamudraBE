using System;
using System.Collections.Generic;

// Code scaffolded by EF Core assumes nullable reference types (NRTs) are not used or disabled.
// If you have enabled NRTs for your project, then un-comment the following line:
// #nullable disable

namespace VSManagement.IOModels
{
    public partial class TblMigrationEmployee
    {
        public int Id { get; set; }
        public int? Empsysid { get; set; }
        public string EmployeeCode { get; set; }
        public string FirstName { get; set; }
        public string LastName { get; set; }
        public string EmpName { get; set; }
        public string FatherName { get; set; }
        public string DateOfBirth { get; set; }
        public string Gender { get; set; }
        public string BloodGroup { get; set; }
        public string IsActive { get; set; }
        public string MaritalStatus { get; set; }
        public string AadharNumber { get; set; }
        public string AadharName { get; set; }
        public string CurrAddress1 { get; set; }
        public string CurrAddress2 { get; set; }
        public string CurrCity { get; set; }
        public string CurrState { get; set; }
        public string CurrPinCode { get; set; }
        public string CurrMobile { get; set; }
        public string CurrHome { get; set; }
        public string CurrOther { get; set; }
        public string CurrEmailWork { get; set; }
        public string CurrEmailPersonal { get; set; }
        public string PrmnentAddress1 { get; set; }
        public string PrmnentAddress2 { get; set; }
        public string PrmnentCity { get; set; }
        public string PrmnentState { get; set; }
        public string PrmnentPinCode { get; set; }
        public string PrmnentMobile { get; set; }
        public string PrmnentHome { get; set; }
        public string PrmnentOther { get; set; }
        public string PrmnentEmailWork { get; set; }
        public string PrmnentEmailPersonal { get; set; }
        public string EmergAddress1 { get; set; }
        public string EmergAddress2 { get; set; }
        public string EmergCity { get; set; }
        public string EmergState { get; set; }
        public string EmergPinCode { get; set; }
        public string EmergMobile { get; set; }
        public string EmergHome { get; set; }
        public string EmergOther { get; set; }
        public string EmergEmailWork { get; set; }
        public string EmergEmailPersonal { get; set; }
        public string EmployeeStatusId { get; set; }
        public string DateOfJoining { get; set; }
        public string ProbationPeriod { get; set; }
        public string ConfirmationDate { get; set; }
        public string FunctionalDesignation { get; set; }
        public string RelevantExperince { get; set; }
        public string PastExperience { get; set; }
        public string TotalExperience { get; set; }
        public string SalaryGroup { get; set; }
        public string NoticePeriod { get; set; }
        public string CompanyName { get; set; }
        public string CompanyCode { get; set; }
        public string DivisionName { get; set; }
        public string ZoneName { get; set; }
        public string BranchName { get; set; }
        public string LocationName { get; set; }
        public string DepartmentName { get; set; }
        public string DeputationName { get; set; }
        public string EmployeeStatusName { get; set; }
        public string DesignationName { get; set; }
        public string FunctionalDesignationName { get; set; }
        public string GradeName { get; set; }
        public string CategoryName { get; set; }
        public string SubDepartmentName { get; set; }
        public string SubGradeName { get; set; }
        public int? Reporting1 { get; set; }
        public int? Reporting2 { get; set; }
        public int? Reporting3 { get; set; }
        public string IfscCode { get; set; }
        public string Address1 { get; set; }
        public string Address2 { get; set; }
        public string BankName { get; set; }
        public string SalaryAccountNo { get; set; }
        public string BankIfsccode { get; set; }
        public string SalaryBankBranch { get; set; }
        public string PaymentMode { get; set; }
        public string Panno { get; set; }
        public string PfApplicable { get; set; }
        public string Pfnumber { get; set; }
        public string PfGroup { get; set; }
        public string PfCeiling { get; set; }
        public string EsicApplicable { get; set; }
        public string EsicNumber { get; set; }
        public string EsicGroup { get; set; }
        public string PtApplicable { get; set; }
        public string Ptgroup { get; set; }
        public string Vpfapplicable { get; set; }
        public string Vpfpercentage { get; set; }
        public string ResignationDate { get; set; }
        public string IsBranchEsicApplicable { get; set; }
        public int? SalaryOnHold { get; set; }
        public int? Isdaily { get; set; }
        public string LastworkingDate { get; set; }
        public string Uannumber { get; set; }
        public string CostCenterName { get; set; }
        public DateTime? CreatedBy { get; set; }
    }
}
